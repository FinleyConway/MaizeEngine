...
namespace Maize
{
    ...
    public:
        ...
        void BeginSession(const std::string& name, const std::string& filepath, uint32_t sizeBeforeDump = 512)
        {
            m_Filepath = filepath;
            ...
            if (m_CurrentSession != nullptr)
            {
                ...
                {
                    CORE_LOG_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->name);
                }
            ...
            else
            {
                if (Log::GetCoreLogger() != nullptr)
                {
                    CORE_LOG_ERROR("Instrumentor could not open file {}.", filepath);
                }
            }
        }
    ...
    private:
        ...
        ~Instrumentor()
        {
            ...
            if (Log::GetCoreLogger() != nullptr)
            {
                CORE_LOG_INFO("https://ui.perfetto.dev/");
            }
        }
    ...
    #define PROFILE_BEGIN_SESSION(name, filepath, dumpThres) ::Maize::Instrumentor::Get().BeginSession(name, filepath, dumpThres)
    #define PROFILE_END_SESSION() ::Maize::Instrumentor::Get().EndSession()
