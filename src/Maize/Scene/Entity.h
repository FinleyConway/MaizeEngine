...
namespace Maize {
	...
	private:
		...
		static Entity CreateEntity(flecs::world& world, Vec2f position, bool isStatic, bool isPersistent, Args&&... args)
		{
			...
			if (isPersistent)
			{
				entity.AddOrReplaceComponent<Persistent>(); // tag that label that this entity will exist across multiple scenes
			}
			else
			{
